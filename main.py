...
def sarsa(episodes=2000, alpha=0.01, gamma=0.9, eps=0.1):
    ...
    return q, policy, rmse_list, policy_list, total_time, total_steps
...
if __name__ == "__main__":
    ...
    for i in range(EPOCHS):
        q, policy, rmse_ql, policy_list, runtime, num_steps = qlearning(episodes=EPISODES,eps=0.01)
        errors_ql.append(rmse_ql)
        runtime_ql.append(runtime)
        num_steps_ql.append(num_steps)

        q, policy, rmse_sarsa, policy_list, runtime, num_steps = sarsa(episodes=EPISODES,eps=0.01)
        errors_sr.append(rmse_sarsa)
        runtime_sr.append(runtime)
        num_steps_sr.append(num_steps)

        q, policy, rmse_mc, policy_list = montecarlo_es(episodes=EPISODES,eps=0.01)
    ...
    plt.plot(num_steps_ql)
    plt.plot(num_steps_sr)
    plt.legend(["QR", "SR"])
    plt.show()
